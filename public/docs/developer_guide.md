...
> **⚠️ Do not edit this file manually.** This guide is maintained by the `npm run update-userguide` workflow. Narrative and screenshots are regenerated from the live app. See “How this guide is generated” below.
...
## How this guide is generated

This document is **auto-generated** and kept in sync with the application via a scripted workflow:

1. **Command:** `npm run update-userguide`
2. **Steps:**
   - Builds the app (`npm run build`)
   - Runs Playwright E2E tests (`e2e/presenterMode.spec.ts`) with `COGNIBIOME_SCREENSHOT_DIR=public/docs/screenshots/app`
   - Writes per-section screenshots (e.g. `01_Dashboard_Overview.png`, `02_Pilot_Overview.png`) into `public/docs/screenshots/app/`
   - Updates the “Screenshots updated” date in this file
   - Commits changes when run locally (skipped in CI via `SKIP_GIT_COMMIT=1`)
3. **Manual workflow (GitHub Actions):** The “Update Developer Guide Screenshots” workflow (`workflow_dispatch`) runs the same steps and uploads the screenshots as an artifact for download.
4. **Cursor rule:** When editing this file, the project rule instructs the agent to run `npm run update-userguide` first, then update narrative only where the UI has changed.

**Why not edit manually?** Screenshots and section flow are tied to the Playwright spec. Manual edits risk drift between the guide and the actual UI. Use the update workflow, then adjust only the specific narrative around changed screenshots.

---

## Table of Contents

0. [How this guide is generated](#how-this-guide-is-generated)
